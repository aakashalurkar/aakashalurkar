...
class ShowProperty extends Component {
  ...
  componentDidMount() {
    ...
    // });
    axios.get('http://localhost:3001/getownerproperty', { params: { data: data } })
      .then((response) => {
        //console.log("Status Code : ",response.data);
        //   if(response.data === 400){
        ...
        <div>
          ...
          <Route render={({ history }) => (
              <Button variant="contained" color="primary" value="Owner" onClick={() => {
                history.push({
                  pathname: "/home",
                  state: { ownerkey: this.state.ownerkey }
                });
              }} >
                SEARCH PROPERTIES
            </Button>
            ...
            <Route render={({ history }) => (
              <Button variant="contained" color="secondary" value="Owner" onClick={() => {
                history.push({
